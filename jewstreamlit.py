...
# --- 頁面設定 ---
st.set_page_config(page_title="金工報價系統 v8.1", layout="centered")
...
with st.expander("💍 改圍加價"):
    ...
    if do_resize:
        sub_c = 300
        if st.checkbox("白金 +100", key='r_plat'): sub_c += 100
        if st.checkbox("寬版 +100", key='r_wide'): sub_c += 100
        if st.checkbox("封底 +100", key='r_back'): sub_c += 100
        if st.checkbox("滿鑽 +100", key='r_full'): sub_c += 100
...
with st.expander("🔨 維修設計"):
    sub_e = 0
    sub_e += st.number_input("C圈1點 ($100)", min_value=0, key='rep_c') * 100
    sub_e += st.number_input("補字印 ($150)", min_value=0, key='rep_s') * 150
    st.divider()
    st.write("**黏耳針 / 補爪 ($200 + 金料)**")
    comb_q = st.number_input("次數", min_value=0, key='comb_q')
    comb_w = st.number_input("金料重 ", min_value=0.0, key='comb_w')
    comb_m = st.radio("材質選擇 ", ["金", "白金"], key='comb_m', horizontal=True)
...
# 準備 CSV 數據 (使用 utf-8-sig 解決 Excel 亂碼)
df = pd.DataFrame([{
...
with col_btn2:
    # 解決手機下載後無法退回的問題：直接使用標準下載按鈕
    st.download_button(
        label="下載報價單 (Excel可開)",
        data=csv_data,
        file_name=f"報價單_{datetime.now().strftime('%m%d')}.csv",
        mime="text/csv",
