...
# --- 頁面設定 ---
st.set_page_config(page_title="金工報價系統 v7.5", layout="centered")
...
with st.expander("💎 鑲嵌服務", expanded=True):
    ...
    st.markdown(f"<h4 style='color: blue;'>鑲嵌小計: ${int(sub_b)}</h4>", unsafe_allow_html=True)
    total_sum += sub_b
...
if col_btn2.button("準備匯出紀錄", use_container_width=True):
    df = pd.DataFrame([{"時間": datetime.now().strftime("%Y-%m-%d %H:%M"), "金價": g_sell, "金額": int(total_sum), "明細": " | ".join(details), "備註": note}])
    csv = io.StringIO()
    df.to_csv(csv, index=False, encoding='utf-8-sig')
    st.download_button("點此下載 CSV", data=csv.getvalue(), file_name="報價紀錄.csv", mime="text/csv")