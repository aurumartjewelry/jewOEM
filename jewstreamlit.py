...
# --- 頁面設定 ---
st.set_page_config(page_title="金工報價系統 v6.8", layout="centered")
...
st.markdown(
    f"""
    <div style="display: flex; justify-content: space-around; background-color: #f0f2f6; padding: 10px; border-radius: 10px;">
...
with st.expander("💎 鑲嵌服務", expanded=True):
    ...
    for i in range(2):
        ...
        if sc > 0:
            price = (get_stone_price(sw) + (50 if sv else 0)) * sc
            sub_b += price
...
with st.expander("💍 改圍加價"):
    ...
    if do_resize:
        ...
        if st.checkbox("改大 +200", key='r_big'):
            sub_c += 200
            bw = st.number_input("金料重量(錢)", min_value=0.0, key='big_w')
...
# E. 維修設計
with st.expander("🔨 維修設計"):
    sub_e = 0
    c_q = st.number_input("C圈1點 ($100)", min_value=0, key='rep_c')
    s_q = st.number_input("補字印 ($150)", min_value=0, key='rep_s')
    sub_e += (c_q * 100) + (s_q * 150)
    
    st.write("黏耳針 / 補爪 ($200 + 金料)")
    col_e1, col_e2, col_e3 = st.columns([1, 1, 1])
    comb_q = col_e1.number_input("次數", min_value=0, key='comb_q')
    comb_w = col_e2.number_input("金料(錢)", min_value=0.0, key='comb_w')
    comb_m = col_e3.radio("材質選擇", ["金", "白金"], key='comb_m', horizontal=True)
    m_p = g_sell if comb_m == "金" else p_sell_pt
    sub_e += (comb_q * 200) + (comb_w * m_p)
