...
if st.session_state.records:
    ...
    def simplify_detail(detail_str):
        if not detail_str or detail_str == "無項目":
            return ""
        items = detail_str.split(" | ")
        simple = []
        for item in items:
            name = re.sub(r'\[.*?\]', '', item)        # 去 [...]
            name = re.sub(r'\(.*?\)', '', name)         # 去 (...)
            name = re.sub(r'\$[\d,]+', '', name)        # 去 $數字
            name = re.sub(r'[\d.]+ct[×xX×]?\d*', '', name)  # 去 0.35ct×1
            name = re.sub(r'[\d.]+錢', '', name)        # 去 1.000錢
            name = re.sub(r'[×xX×]\d+', '', name)      # 去 ×3
            name = re.sub(r'18K|14K|9K|Pt\d*|\b750\b|\b585\b|\b375\b|\b950\b', '', name)
            name = re.sub(r'[\d.]+', '', name)          # 去殘留數字
            name = re.sub(r'[=+\-\s｜|,，]+$', '', name)  # 去尾端符號
            name = re.sub(r'^[=+\-\s｜|,，]+', '', name)  # 去開頭符號
            name = name.strip()
            if name:
                simple.append(name)
        return "｜".join(simple)
    ...
    with pd.ExcelWriter(out_simple, engine='openpyxl') as writer:
        ...
        # 逐列寫入彙總，避免 Excel 陣列公式的 @ 問題
        for i, row_data in enumerate(export_records):
            data_row = i + 2  # header 在第1列，資料從第2列
            name_val  = row_data.get("備註", "") or ""
            total_val = row_data.get("總金額", 0)
            labor_val = row_data.get("工費金額", 0)
            detail_val = simplify_detail(row_data.get("明細", ""))
            detail_lines = detail_val.replace("｜", "\n")
            ...
            cell = ws.cell(row=summary_row + i, column=1, value=text)
            cell.alignment = Alignment(wrap_text=True)
            ws.row_dimensions[summary_row + i].height = max(60, 15 * (detail_val.count("｜") + 4))
