...
with st.expander("💎 鑲嵌服務", expanded=False):
    ...
    for i in range(dn):
        ...
        if sw > 0 and sc > 0:
            ...
            # sw == 0，key 還是要渲染避免 session state 問題
            _ = c3.checkbox("造型爪+50", key=f'sv_{i}', disabled=True)
            _ = c3.checkbox("寶石類",   key=f'is_gem_{i}', disabled=True)
            _ = c3.checkbox("水晶類",   key=f'is_cry_{i}', disabled=True)
