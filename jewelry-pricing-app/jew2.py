...
# 數量輸入
st.write("🔢 **數量**")
qty_col1, qty_col2 = st.columns([1, 3])
order_qty = qty_col1.number_input("件數", min_value=1, step=1, value=1, key='order_qty')
if order_qty > 1:
    qty_col2.markdown(f"<div style='padding-top:28px;font-size:15px;'>單件 <b>${int(total_final)}</b> × {order_qty} 件</div>", unsafe_allow_html=True)
...
if st.session_state.records:
    ...
    for idx, rec in enumerate(records):
        ...
        # 金價行（排最後）
        gold_line = ""
        if gold_str or plat_str:
            gold_line = f"金價：黃金 ${gold_str}　鉑金 ${plat_str}"
