...
with st.expander("💍 改圍加價", expanded=False):
    ...
    if do_resize:
        ...
        if st.checkbox("改大 +200", key='r_big'):
            sub_c += 200
            bw = st.number_input("金料(錢):", min_value=0.0, step=0.001, format="%.3f", key='big_w')
...
with st.expander("🔨 維修設計", expanded=False):
    ...
    comb_w = ce2.number_input("金料(錢)", min_value=0.0, step=0.001, format="%.3f", key='comb_w')
    comb_m = ce3.radio("材質選擇", ["金", "白金"], key='comb_m', horizontal=True)
    sub_e += (comb_q * 150) + (comb_w * (g_sell if comb_m == "金" else p_sell_pt))

    st.write("---")
    st.write("**補金 ($自填工資 + 金料)**")
    le1, le2, le3 = st.columns([1, 1, 1])
    l_price = le1.number_input("補金工資($)", min_value=0.0, step=10.0, key='laser_p')
    l_weight = le2.number_input("金料(錢) ", min_value=0.0, step=0.001, format="%.3f", key='laser_w')
...
# C. 改圍：拆出金料 vs 工費
#    改圍金料部分（改大時的 bw * 金價）需分開
resize_gold = 0
resize_labor = 0
if st.session_state.get('do_resize', False):
    resize_labor_base = sub_c
    # 金料金額（改大時）
    bw_val = st.session_state.get('big_w', 0.0)
    r_big_val = st.session_state.get('r_big', False)
    b_mat_val = st.session_state.get('big_mat', '金')
    if r_big_val:
        resize_gold = bw_val * (g_sell if b_mat_val == "金" else p_sell_pt)
        resize_labor_base = sub_c - resize_gold
    resize_labor = resize_labor_base
    resize_gold = sub_c - resize_labor_base
final_c = int(resize_labor * walkin_mult) + int(resize_gold)

# D. 拋光電鍍：全部工費 × 倍率
final_d = int(sub_d * walkin_mult)

# E. 維修設計：拆出金料（補爪金料 + 補金金料），工費倍率
comb_w_val   = st.session_state.get('comb_w', 0.0)
comb_m_val   = st.session_state.get('comb_m', '金')
laser_w_val  = st.session_state.get('laser_w', 0.0)
laser_m_val  = st.session_state.get('laser_m', '金')
repair_gold  = (comb_w_val * (g_sell if comb_m_val == "金" else p_sell_pt)) + \
               (laser_w_val * (g_sell if laser_m_val == "金" else p_sell_pt))
