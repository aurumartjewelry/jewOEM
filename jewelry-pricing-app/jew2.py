    ...
    for i in range(dn):
        ...
        if sw > 0 and sc > 0:
            if sw <= 0.09:
                ...
                is_natural = False
                if sw >= 1.0:
                    is_natural = c3.checkbox("天然（×2）", key=f'is_natural_{i}')
                    if is_natural:
            ...
            if is_gem:
                ...
                st.caption(f"水晶類　{hint}")
            if sw >= 1.0:
                c3.checkbox("天然（×2）", key=f'is_natural_{i}')
            else:
                st.session_state.setdefault(f'is_natural_{i}', False)
            sv = c4.checkbox("造型爪+50", key=f'sv_{i}')
        else:
            # sw == 0，key 預設確保存在，不渲染 widget
            st.session_state.setdefault(f'sv_{i}', False)
...
# ── 拆分工費 / 金料 / 裸石 ──
total_bare   = int(sub_b2)
total_gold   = sub_a_gold + int(resize_gold) + int(repair_gold)   # 不含裸石
total_labor  = total_final - total_gold - total_bare
...
col_tf.metric("🏷️ 總金額",  f"${int(total_final)}")
...
if st.session_state.records:
    ...
    # ── 精簡明細：只保留項目名稱 ──
    import re
    def simplify_detail(detail_str):
        return detail_str  # 已由 simple_details 直接提供，此函數保留備用

    # 欄位：A=名稱 B=總金額 C=工費金額 D=明細
    ...
    out_simple = io.BytesIO()
    import openpyxl
    from openpyxl.styles import Alignment
    with pd.ExcelWriter(out_simple, engine='openpyxl') as writer:
        df_simple.to_excel(writer, index=False, sheet_name='精簡報價單')
        wb = writer.book
        ws = writer.sheets['精簡報價單']
        n = len(df_simple)
        summary_row = n + 3

        # 每列寫一個 TEXTJOIN 公式，引用同列的 A/B/C/D 欄（單儲存格，不用陣列範圍，無 @ 問題）
        for i in range(n):
            data_row = i + 2  # header 第1列，資料從第2列
            formula = (
                f'=A{data_row}&CHAR(10)&'
                f'"總金額："&B{data_row}&CHAR(10)&'
                f'"工費："&C{data_row}&CHAR(10)&'
                f'"明細："&SUBSTITUTE(D{data_row},"｜",CHAR(10))'
            )
            cell = ws.cell(row=summary_row + i, column=1, value=formula)
            cell.alignment = Alignment(wrap_text=True)
            ws.row_dimensions[summary_row + i].height = max(60, 15 * (df_simple.iloc[i]["明細"].count("｜") + 4))
    ...
    with dl2:
        st.download_button(
            label=f"📄 精簡報價單（{'勾選' if selected else '全部'}）",
            data=out_simple,
            file_name=f"精簡報價_{datetime.now().strftime('%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )
