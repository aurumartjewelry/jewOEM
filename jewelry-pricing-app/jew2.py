...
with st.expander("🛠️ 執模台工", expanded=False):
    ...
    if do_mold:
        ...
        if col_add_gw.button("＋ 新增金料", key='add_gw'):
            st.session_state.gw_rows += 1; st.rerun()
        if gw_n > 1 and col_del_gw.button("－ 移除最後一組", key='del_gw'):
            st.session_state.gw_rows -= 1; st.rerun()
        ...
        if col_add_m.button("＋ 新增件數", key='add_mold'):
            st.session_state.mold_rows += 1; st.rerun()
        if mold_n > 1 and col_del_m.button("－ 移除最後一件", key='del_mold'):
            st.session_state.mold_rows -= 1; st.rerun()
...
with st.expander("💎 鑲嵌服務", expanded=False):
    ...
    if col_add_d.button("＋ 新增標鑲", key='add_diamond'):
        st.session_state.diamond_rows += 1; st.rerun()
    if dn > 1 and col_del_d.button("－ 移除", key='del_diamond'):
        st.session_state.diamond_rows -= 1; st.rerun()
    ...
    if col_add_g.button("＋ 新增寶石", key='add_gem'):
        st.session_state.gem_rows += 1; st.rerun()
    if gn > 1 and col_del_g.button("－ 移除 ", key='del_gem'):
        st.session_state.gem_rows -= 1; st.rerun()
    ...
    if col_add_c.button("＋ 新增水晶", key='add_crystal'):
        st.session_state.crystal_rows += 1; st.rerun()
    if cn > 1 and col_del_c.button("－ 移除  ", key='del_crystal'):
        st.session_state.crystal_rows -= 1; st.rerun()
...
with st.expander("💍 裸石價格", expanded=False):
    ...
    if col_add_b.button("＋ 新增裸石", key='add_bare'):
        st.session_state.bare_rows += 1; st.rerun()
    if bn > 1 and col_del_b.button("－ 移除   ", key='del_bare'):
        st.session_state.bare_rows -= 1; st.rerun()
