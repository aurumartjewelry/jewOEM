...
with st.expander("✨ 拋光 / 電鍍", expanded=False):
    ...
    # 局部拋光 / 單耳 / 單鑲口
    c1, c2 = st.columns(2)
    is_partial = c1.checkbox("局部拋光／單耳／單鑲口 $100", key='p_partial')
    if is_partial:
        sub_d += 100
        if c2.checkbox("附加電鍍 +$50", key='p_partial_plate'):
            sub_d += 50

    # 基礎拋光
    c3, c4 = st.columns(2)
    is_basic = c3.checkbox("基礎拋光 $200", key='p_basic')
    if is_basic:
        sub_d += 200
        if c4.checkbox("附加電鍍 +$50 ", key='p_basic_plate'):
            sub_d += 50
    ...
    # 項鍊 / 手鍊
    c5, c6 = st.columns(2)
    is_necklace = c5.checkbox("項鍊／手鍊 $250", key='p_necklace')
    if is_necklace:
        sub_d += 250
        if c6.checkbox("附加電鍍 +$100", key='necklace_plate'):
            sub_d += 100

    # 手鐲
    c7, c8 = st.columns(2)
    is_bangle = c7.checkbox("手鐲 $300", key='p_bangle')
    if is_bangle:
        sub_d += 300
        if c8.checkbox("附加電鍍 +$100 ", key='bangle_plate'):
            sub_d += 100
