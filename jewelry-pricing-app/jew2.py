...
def reset_fields():
    ...
    st.session_state.gw_mat1 = "18K(750)"; st.session_state.gw_mat2 = "18K(750)"
...
with st.expander("🛠️ 執模台工", expanded=False):
    ...
    if do_mold:
        ...
        gw_mat1  = gw_col1.selectbox("材質 A", ["18K(750)", "14K(585)", "9K(375)", "Pt950"], key='gw_mat1')
        gw_w1    = gw_col2.number_input("重量 A (錢)", min_value=0.0, step=0.001, format="%.3f", key='gw_w1')
        gw_loss1 = gw_col3.number_input("損耗 A", min_value=1.0, max_value=2.0, value=1.35, step=0.01, key='gw_loss1')
        ...
        gw_mat2  = gw_col5.selectbox("材質 B", ["18K(750)", "14K(585)", "9K(375)", "Pt950"], key='gw_mat2')
        gw_w2    = gw_col6.number_input("重量 B (錢)", min_value=0.0, step=0.001, format="%.3f", key='gw_w2')
        gw_loss2 = gw_col7.number_input("損耗 B", min_value=1.0, max_value=2.0, value=1.35, step=0.01, key='gw_loss2')
...
with st.expander("💎 鑲嵌服務", expanded=False):
    ...
    st.divider()
    st.write("### 寶石類 (紅藍寶、碧璽)")
    st.caption("5ct以下 $400｜5~10ct $500｜10ct以上 $600")
    for i in range(3):
        gs1, gs2, gs3, gs4 = st.columns([2, 2, 2, 2])
        g_w = gs1.number_input(f"重量(ct) #{i+1}", min_value=0.0, step=0.001, format="%.3f", key=f'gem_w_{i}')
        g_c = gs2.number_input(f"顆數 #{i+1}", min_value=0, step=1, key=f'gem_c_{i}')
        g_v = gs3.checkbox(f"造型爪+50 #{i+1}", key=f'gem_v_{i}')
        if g_c > 0 and g_w > 0:
            if g_w <= 5: g_unit = 400
            elif g_w <= 10: g_unit = 500
            else: g_unit = 600
            g_p = (g_unit + (50 if g_v else 0)) * g_c
            sub_b += g_p
            gs4.metric("小計", f"${g_p}")
            details.append(f"寶石鑲{g_w:.3f}ct×{g_c}顆(${g_p})")

    st.divider()
    st.write("### 水晶類")
    st.caption("5ct以下 $300｜5~10ct $400｜10ct以上 $500")
    for i in range(3):
        cs1, cs2, cs3, cs4 = st.columns([2, 2, 2, 2])
        c_w = cs1.number_input(f"重量(ct) #{i+1}", min_value=0.0, step=0.001, format="%.3f", key=f'cry_w_{i}')
        c_c = cs2.number_input(f"顆數 #{i+1}", min_value=0, step=1, key=f'cry_c_{i}')
        c_v = cs3.checkbox(f"造型爪+50 #{i+1}", key=f'cry_v_{i}')
        if c_c > 0 and c_w > 0:
            if c_w <= 5: c_unit = 300
            elif c_w <= 10: c_unit = 400
            else: c_unit = 500
            c_p = (c_unit + (50 if c_v else 0)) * c_c
            sub_b += c_p
            cs4.metric("小計", f"${c_p}")
            details.append(f"水晶鑲{c_w:.3f}ct×{c_c}顆(${c_p})")
...
with st.expander("💍 裸石價格", expanded=False):
    ...
    for j in range(3):
        cp_col, cw_col, ct_col = st.columns([2, 2, 2])
        cp = cp_col.number_input(f"單價/ct #{j+1}", min_value=0.0, key=f'cp_{j}')
        cw = cw_col.number_input(f"總重量(ct) #{j+1}", min_value=0.0, step=0.001, format="%.3f", key=f'cw_{j}')
        ct_type = ct_col.selectbox(f"類型 #{j+1}", ["天然", "培育"], key=f'ct_type_{j}')
        if cp > 0 and cw > 0:
            amt = cp * cw
            sub_b2 += amt
...
# 3D掃描、製圖（在 sub_e 裡）×2 需特別處理
scan_fee = 500 if st.session_state.get('3d_scan', False) else 0
draw_fee_val = st.session_state.get('draw_fee', 0)
# 已含在 repair_labor 中，需補差額（×2 vs ×3）
scan_draw_extra = int((scan_fee + draw_fee_val) * (2 - 3) * (1 if is_walkin else 0))
# 掃描/製圖應 ×2，但被 repair_labor ×3 多算了，補回差額
final_e = final_e + scan_draw_extra
