...
if st.session_state.records:
    ...
    def simplify_detail(detail_str):
        ...
        for item in items:
            ...
            # 去掉數字×數字、成色、錢、損耗等
            name = re.sub(r'\d+\.?\d*ct×?\d*', '', name)
            name = re.sub(r'\d+\.?\d*錢', '', name)
            name = re.sub(r'×\d+', '', name)
    ...
    with pd.ExcelWriter(out_simple, engine='openpyxl') as writer:
        ...
        n = len(df_simple)  # 資料列數（不含標題）
        # 彙總公式放在資料下方空一列
        summary_row = n + 3
        # B2:B{n+1} 對應名稱欄（header 在第1列，資料從第2列）
        r_start = 2
        r_end   = n + 1
        formula = (
            f'=TEXTJOIN(CHAR(10)&CHAR(10),TRUE,'
            f'"名稱："&B{r_start}:B{r_end}&CHAR(10)&'
            f'"總金額："&C{r_start}:C{r_end}&CHAR(10)&'
            f'"工費："&D{r_start}:D{r_end}&CHAR(10)&'
            f'"明細："&SUBSTITUTE(E{r_start}:E{r_end},"｜",CHAR(10)))'
        )
