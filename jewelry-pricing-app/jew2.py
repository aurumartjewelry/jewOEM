...
if st.session_state.records:
    ...
    for idx, rec in enumerate(records):
        ...
        with col_info:
            ...
            st.markdown(
                ...
                f"備註：{rec.get('備註','') or '—'}　"
                f"**${rec.get('總金額',0)}**　"
                f"<span style='font-size:12px;color:gray'>{brief_detail[:60]}{'…' if len(brief_detail)>60 else ''}</span>",
                unsafe_allow_html=True
            )
    ...
    with btn1:
        if st.button("🗑️ 刪除勾選項目", use_container_width=True):
            if selected:
                ...
                st.session_state.selected_records = set()
                ok = save_records_to_github(new_records, st.session_state.gh_sha)
                st.rerun()
    with btn2:
    ...
    with pd.ExcelWriter(out_simple, engine='openpyxl') as writer:
        ws_name = '精簡報價單'
        df_simple.to_excel(writer, index=False, sheet_name=ws_name)
        wb = writer.book
        ws = writer.sheets[ws_name]
        # 在最後一列下方加入 TEXTJOIN 彙總公式（模仿圖片格式）
        n = len(df_simple) + 1  # +1 for header
        summary_row = n + 2
        # 公式：每筆依序組成「名稱：X 總金額：Y 工費：Z 明細：W」多行文字
        ranges = {
            'B': f"B2:B{n+1}",
            'C': f"C2:C{n+1}",
            'D': f"D2:D{n+1}",
        ...
        formula = (
            ...
            f'"名稱："&{ranges["B"]}&CHAR(10)&'
            f'"總金額："&{ranges["C"]}&CHAR(10)&'
            f'"工費："&{ranges["D"]}&CHAR(10)&'
            f'"明細："&SUBSTITUTE({ranges["E"]},"｜",CHAR(10)))'
        )
        ws[f'A{summary_row}'] = formula
        ws[f'A{summary_row}'].alignment = __import__('openpyxl').styles.Alignment(wrap_text=True)
    out_simple.seek(0)
    ...
    with dl2:
        label2 = f"📄 精簡報價單（{'勾選' if selected else '全部'}）"
        st.download_button(
            label=label2,
            data=out_simple,
