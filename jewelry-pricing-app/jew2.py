...
# 若有重設旗標，清除所有動態 widget key 再繼續渲染
if st.session_state.get('_do_reset', False):
    ...
    # 重設預設值
    st.session_state.mold_m1        = "K金"
    st.session_state.mold_m2        = "K金"
    st.session_state.eng_choice     = "無"
    st.session_state.polish_choice  = "無"
    st.session_state.gw_mat1        = "18K(750)"
    st.session_state.gw_mat2        = "18K(750)"
    # 動態列數回 1
    for _k in ('gw_rows', 'mold_rows', 'diamond_rows', 'gem_rows', 'crystal_rows', 'bare_rows'):
        st.session_state[_k] = 1

def reset_fields():
    """設置重設旗標，rerun 後生效（避免 widget 渲染衝突）"""
...
with st.expander("🛠️ 執模台工", expanded=False):
    ...
    if do_mold:
        ...
        col_add_gw.button("＋ 新增金料", key='add_gw',
            on_click=lambda: st.session_state.update({'gw_rows': st.session_state.gw_rows + 1}))
        if gw_n > 1:
            col_del_gw.button("－ 移除最後一組", key='del_gw',
                on_click=lambda: st.session_state.update({'gw_rows': st.session_state.gw_rows - 1}))
        ...
        col_add_m.button("＋ 新增件數", key='add_mold',
            on_click=lambda: st.session_state.update({'mold_rows': st.session_state.mold_rows + 1}))
        if mold_n > 1:
            col_del_m.button("－ 移除最後一件", key='del_mold',
                on_click=lambda: st.session_state.update({'mold_rows': st.session_state.mold_rows - 1}))

        is_combo = False
        if sum(1 for p in mold_parts if p[0] > 0) >= 2:
            is_combo = st.checkbox("🔗 雙色組合件 +$500", key='mold_combo')

        # 只取前兩件計算工費（函數設計如此），超過2件的加總
        w_list = [(p[0], p[1]) for p in mold_parts if p[0] > 0]
        if len(w_list) == 0:
            sub_a_labor = 0
        elif len(w_list) == 1:
            sub_a_labor = calculate_jewelry_fee(w_list[0][0], w_list[0][1], 0, "K金", False)
        else:
            sub_a_labor = calculate_jewelry_fee(w_list[0][0], w_list[0][1], w_list[1][0], w_list[1][1], is_combo)
            for extra in w_list[2:]:
                sub_a_labor += calculate_jewelry_fee(extra[0], extra[1], 0, "K金", False)

        mold_desc = "+".join(f"{p[1]}{p[0]:.3f}錢" for p in mold_parts if p[0] > 0)
        if is_combo: mold_desc += "(雙色組合)"
...
with st.expander("💎 鑲嵌服務", expanded=False):
    ...
    for i in range(dn):
        ...
        if sc > 0 and sw > 0:
            ...
            details.append(f"標鑲{sw:.3f}ct×{sc}")
    col_add_d, col_del_d = st.columns(2)
    col_add_d.button("＋ 新增標鑲", key='add_diamond',
        on_click=lambda: st.session_state.update({'diamond_rows': st.session_state.diamond_rows + 1}))
    if dn > 1:
        col_del_d.button("－ 移除", key='del_diamond',
            on_click=lambda: st.session_state.update({'diamond_rows': st.session_state.diamond_rows - 1}))
    ...
    col_add_g.button("＋ 新增寶石", key='add_gem',
        on_click=lambda: st.session_state.update({'gem_rows': st.session_state.gem_rows + 1}))
    if gn > 1:
        col_del_g.button("－ 移除 ", key='del_gem',
            on_click=lambda: st.session_state.update({'gem_rows': st.session_state.gem_rows - 1}))
    ...
    col_add_c.button("＋ 新增水晶", key='add_crystal',
        on_click=lambda: st.session_state.update({'crystal_rows': st.session_state.crystal_rows + 1}))
    if cn > 1:
        col_del_c.button("－ 移除  ", key='del_crystal',
            on_click=lambda: st.session_state.update({'crystal_rows': st.session_state.crystal_rows - 1}))
...
with st.expander("💍 裸石價格", expanded=False):
    ...
    col_add_b.button("＋ 新增裸石", key='add_bare',
        on_click=lambda: st.session_state.update({'bare_rows': st.session_state.bare_rows + 1}))
    if bn > 1:
        col_del_b.button("－ 移除   ", key='del_bare',
            on_click=lambda: st.session_state.update({'bare_rows': st.session_state.bare_rows - 1}))
...
with st.expander("🔨 維修", expanded=False):
    ...
    st.write("---")
    eng_choice = st.radio("刻字服務", ["無", "1-5字($50)", "5-10字/LOGO($100)", "11-20字($200)"], key='eng_choice', horizontal=True)
    if "1-5" in eng_choice: sub_e += 50
    elif "5-10" in eng_choice: sub_e += 100
    elif "11-20" in eng_choice: sub_e += 200
...
# 散客加價
st.divider()
is_walkin = st.checkbox("👤 散客報價", key='is_walkin')

# 各類別散客倍率
# 執模、鑲嵌、維修(刻字/補金/黏耳針) × 3
# 改圍、電鍍 × 2.5
# 3D掃描、製圖 × 2
# 裸石、金料 不加倍
# 補金金料散客時損耗額外增加：18K/14K +0.25，Pt950 +0.3

def walkin_loss_add(mat):
    """散客模式下損耗加成"""
    if not is_walkin: return 0.0
    if mat == "Pt950": return 0.3
    return 0.25  # 18K, 14K, 9K

# A. 執模：工費 ×3，金重不變
final_a = int(sub_a_labor * (3 if is_walkin else 1)) + sub_a_gold

# B. 鑲嵌（鑽石/寶石/水晶/手動）：工費 ×3
final_b = int(sub_b * (3 if is_walkin else 1))
...
final_c = int(resize_labor * (2.5 if is_walkin else 1)) + int(resize_gold)

# D. 拋光電鍍：工費 ×2.5
final_d = int(sub_d * (2.5 if is_walkin else 1))

# E. 維修設計：拆出金料，工費 ×3；散客時補金金料損耗加成
...
final_e = int(repair_labor * (3 if is_walkin else 1)) + int(repair_gold)

# B2. 裸石：不套倍率
final_b2 = int(sub_b2)

# F. 3D出蠟：蠟料不加倍，圖面處理費/掃描/製圖工費 ×2
...
if do_wax_val:
    ...
    final_f = wax_mat_cost + int(design_fee_each * (2 if is_walkin else 1))
else:
    final_f = 0

final_f += int(scan_fee_val * (2 if is_walkin else 1))
final_f += int(draw_fee_val * (2 if is_walkin else 1))
...
if is_walkin:
    st.markdown(
        f"<div style='background:#fff0f0;border-left:4px solid #e00;padding:8px 14px;border-radius:6px;margin:4px 0;color:#7a0000;font-size:13px;'>"
        f"👤 散客報價｜執模/鑲嵌/維修 ×3　改圍/電鍍 ×2.5　3D掃描/製圖 ×2　金料損耗+0.25（Pt+0.3）"
