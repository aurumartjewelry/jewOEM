...
# --- 頁面設定 ---
st.set_page_config(page_title="金工報價系統 v10", layout="centered")
...
if 'records' not in st.session_state: st.session_state.records = [] # 儲存多筆紀錄
...
with st.expander("🔨 維修設計", expanded=False):
    ...
    if st.checkbox("旋轉台(+250)", key='rotary'): sub_e += 250
    
    c_m1, c_m2 = st.columns(2)
    if c_m1.checkbox("特殊圖案(+$500)", key='pattern'): sub_e += 500
    if c_m2.checkbox("3D掃描(+500)", key='3d_scan'): sub_e += 500
