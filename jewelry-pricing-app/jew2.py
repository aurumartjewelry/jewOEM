...
if st.session_state.records:
    ...
    def simplify_detail(detail_str):
        ...
        for item in items:
            ...
            name = re.sub(r'18K|14K|9K|Pt\d*|750|585|375|950', '', name)  # 去成色
    ...
    # 欄位：A=名稱 B=總金額 C=工費金額 D=明細（共4欄，index=False）
    ...
    with pd.ExcelWriter(out_simple, engine='openpyxl') as writer:
        ...
        r_start = 2
        r_end   = n + 1
        # A=名稱 B=總金額 C=工費金額 D=明細
        formula = (
            f'=TEXTJOIN(CHAR(10)&CHAR(10),TRUE,'
            f'"名稱："&A{r_start}:A{r_end}&CHAR(10)&'
            f'"總金額："&B{r_start}:B{r_end}&CHAR(10)&'
            f'"工費："&C{r_start}:C{r_end}&CHAR(10)&'
            f'"明細："&SUBSTITUTE(D{r_start}:D{r_end},"｜",CHAR(10)))'
        )
        cell = ws.cell(row=summary_row, column=1, value=formula)
        cell.alignment = Alignment(wrap_text=True)
        ws.row_dimensions[summary_row].height = 15 * (n + 1)
