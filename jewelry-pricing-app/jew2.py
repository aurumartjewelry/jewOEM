...
def get_stone_price(w):
    ...
    if w <= 1.99: return 400
    if w <= 2.99: return 600
    return 800
...
for _i in range(20):
    for _k, _v in [
        ...
        (f'silver_{_i}',  False),
        (f'row_{_i}',     False),
        (f'return_gold_{_i}', False),
        (f'is_natural_{_i}',  False),
        (f'is_cultured_{_i}', False),
        (f'is_emerald_{_i}',  False),
        (f'is_gem_{_i}',  False),
        (f'is_cry_{_i}',  False),
        (f'gem_w_{_i}',   0.0),
        (f'gem_c_{_i}',   0),
...
# 非動態列的單次欄位初始化
for _k, _v in [('manual_labor_price', 0), ('manual_labor_weight', 0.0),
               ('recv_weight', 0.0), ('ship_weight', 0.0)]:
    if _k not in st.session_state:
        st.session_state[_k] = _v
...
# --- 3. UI 介面 ---
st.markdown("<h1 style='text-align: center;'>服務項目報價</h1>", unsafe_allow_html=True)
...
with fetch_col:
    if st.button("🔄 自動抓取詮美今日金價", use_container_width=True):
        ...
        if err:
            st.session_state.fetch_msg = "失敗"
            st.session_state.fetch_ok = False
        else:
            st.session_state.gold_price = float(g_fetched)
            st.session_state.plat_price = float(p_fetched)
            st.session_state.fetch_msg = "成功"
            st.session_state.fetch_ok = True
            fetch_allbeauty_prices.clear()
            st.rerun()

with name_col:
    customer_name = st.text_input("👤 客戶名稱", key='customer_name', placeholder="輸入客戶名稱（選填）")
...
if st.session_state.fetch_msg:
    ...
    st.markdown(
        f"<div style='background:{bg};border-left:4px solid {color};padding:4px 10px;"
        f"border-radius:6px;color:{color};font-size:12px;margin-bottom:6px;display:inline-block;'>"
        f"金價抓取：{st.session_state.fetch_msg}</div>",
...
st.markdown(
    f"""
    <div style="display: flex; justify-content: space-around; background-color: #f0f2f6; padding: 10px; border-radius: 10px; margin-bottom: 12px;">
    ...
    if do_mold:
        ...
        if len(active_parts) >= 2:
            combo_choice = st.radio(
                "🔗 組合件加價",
                ["無", "單邊組合 +$300", "雙邊組合 +$500", "封網底 +$500"],
                key='mold_combo', horizontal=True
            )
            if "單邊" in combo_choice: combo_fee = 300
            elif "雙邊" in combo_choice: combo_fee = 500
            elif "封網底" in combo_choice: combo_fee = 500
        ...
        # 手動補充工費（不走自動計算時可直接填入）
        st.write("---")
        st.caption("手動輸入工費（選填，會加入工費合計）")
        man_col1, man_col2, man_col3 = st.columns(3)
        manual_labor_price  = man_col1.number_input("手動工費 ($)", min_value=0, step=50, key='manual_labor_price')
        manual_labor_weight = man_col2.number_input("對應重量 (錢)", min_value=0.0, step=0.001, format="%.3f", key='manual_labor_weight')
        if manual_labor_price > 0:
            sub_a_labor += manual_labor_price
            man_col3.metric("加入工費", f"${manual_labor_price}")
            if manual_labor_weight > 0:
                mold_desc_extra = f"手動({manual_labor_weight:.3f}錢,工${manual_labor_price})"
            else:
                mold_desc_extra = f"手動工費${manual_labor_price}"
        else:
            mold_desc_extra = ""

        mold_desc = "+".join(f"{p[1]}{p[0]:.3f}錢" for p in mold_parts if p[0] > 0)
        if mold_desc_extra: mold_desc += ("+" if mold_desc else "") + mold_desc_extra
...
def get_crystal_price(w):
    """水晶類單價"""
    if w <= 5:   return 300
    if w <= 10:  return 400
    return 500

def gem_bracket_hint(w, price_fn):
    """產生前後價位帶提示文字"""
    brackets = price_fn.__doc__  # 用 docstring 區分
    if price_fn == get_gem_price:
        tiers = [(5, 400), (10, 500), (999, 600)]
        names = ["≤5ct $400", "5~10ct $500", ">10ct $600"]
    else:
        tiers = [(5, 300), (10, 400), (999, 500)]
        names = ["≤5ct $300", "5~10ct $400", ">10ct $500"]
    # 找目前所在 tier index
    idx = 0
    for j, (limit, _) in enumerate(tiers):
        if w <= limit:
            idx = j
            break
    parts = []
    if idx > 0:        parts.append(f"⬆ {names[idx-1]}")
    parts.append(f"▶ **{names[idx]}**")
    if idx < len(names)-1: parts.append(f"⬇ {names[idx+1]}")
    return "　".join(parts)
    ...
    for i in range(dn):
        ...
        if sw > 0 and sc > 0:
            if sw <= 0.09:
                ...
                is_return_gold = opt_col2.checkbox("返金（+$15/顆）",  key=f'return_gold_{i}')
                st.session_state.setdefault(f'sv_{i}', False)
                st.session_state.setdefault(f'is_natural_{i}', False)
                if is_clamp:    unit = 50
                elif is_silver: unit = 45
                else:           unit = get_stone_price(sw)
                if is_row:         unit += 15
                if is_return_gold: unit += 15
                p   = unit * sc
                tag = ("夾鑲" if is_clamp else ("銀飾" if is_silver else "標鑲"))
                if is_row:          tag += "+排鑽"
                if is_return_gold:  tag += "+返金"

            else:
                # ── 0.09以上：可選寶石類或水晶類（互斥），否則走標準鑽石價 ──
                is_gem = c3.checkbox("寶石類", key=f'is_gem_{i}')
                is_cry = c3.checkbox("水晶類", key=f'is_cry_{i}') if not is_gem else False

                if is_gem:
                    base_price = get_gem_price(sw)
                    hint = gem_bracket_hint(sw, get_gem_price)
                    st.caption(f"寶石（紅藍寶、碧璽）{hint}")
                    tag = "寶石鑲"
                elif is_cry:
                    base_price = get_crystal_price(sw)
                    hint = gem_bracket_hint(sw, get_crystal_price)
                    st.caption(f"水晶類　{hint}")
                    tag = "水晶鑲"
                else:
                ...
                is_natural  = type_col1.checkbox("天然（×2）",   key=f'is_natural_{i}')
                is_cultured = type_col2.checkbox("培育（≤600）", key=f'is_cultured_{i}') if not is_natural else False
                is_emerald  = type_col3.checkbox("祖母綠（寶石×2）", key=f'is_emerald_{i}') if not is_natural and not is_cultured else False

                if is_emerald:
                    base_price = get_gem_price(sw) * 2
                    tag += "(祖母綠)"
                elif is_natural:
                    base_price = base_price * 2
                    tag += "(天然)"
                elif is_cultured:
                    base_price = min(base_price, 600)
                    tag += "(培育)"

                sv = c4.checkbox("造型爪+50", key=f'sv_{i}')
                unit = base_price + (50 if sv else 0)
                p    = unit * sc
            ...
            simple_details.append(tag)

        elif sw > 0 and sw > 0.09:
            is_gem = c3.checkbox("寶石類", key=f'is_gem_{i}')
            is_cry = c3.checkbox("水晶類", key=f'is_cry_{i}') if not is_gem else False
            if is_gem:
                hint = gem_bracket_hint(sw, get_gem_price)
            ...
            st.session_state.setdefault(f'sv_{i}', False)
            st.session_state.setdefault(f'is_gem_{i}', False)
            st.session_state.setdefault(f'is_cry_{i}', False)
            st.session_state.setdefault(f'is_natural_{i}', False)
            st.session_state.setdefault(f'is_cultured_{i}', False)
            st.session_state.setdefault(f'is_emerald_{i}', False)
...
with st.expander("💍 改圍加價", expanded=False):
    ...
    if do_resize:
        ...
        if st.checkbox("改大 +200", key='r_big'):
            ...
            sub_c += b_amt
            resize_gold_desc = f"{b_mat2}{bw:.3f}錢×{b_loss}=${b_amt}" if bw > 0 else ""
        else:
            resize_gold_desc = ""
        if resize_gold_desc:
            details.append(f"改圍(工費${int(sub_c - b_amt)}|金{resize_gold_desc}|總${int(sub_c)})")
        else:
            details.append(f"改圍(${int(sub_c)})")
        simple_details.append("改圍")
...
with st.expander("🔨 維修", expanded=False):
    ...
    rep_c_val = col_e1.number_input("鈎鍊+雷射 ($100)", min_value=0, step=1, key='rep_c')
    sub_e += rep_c_val * 100
    if rep_c_val > 0: repair_detail_items.append(f"鈎鍊+雷射×{rep_c_val}")

    rep_weld_val = col_e2.number_input("明火焊接 ($200)", min_value=0, step=1, key='rep_weld')
    sub_e += rep_weld_val * 200
    if rep_weld_val > 0: repair_detail_items.append(f"明火焊接×{rep_weld_val}")

    laser_fee_val = col_e3.number_input("焊接自訂金額 ($)", min_value=0.0, step=10.0, key='entry_laser_fee')
    sub_e += laser_fee_val
    if laser_fee_val > 0: repair_detail_items.append(f"焊接${int(laser_fee_val)}")
    ...
    if comb_q > 0:
        ...
        simple_details.append(f"補爪×{comb_q}")
        repair_detail_items.append(f"補爪×{comb_q}")
    ...
    if l_weight > 0 or l_price > 0:
        ...
        simple_details.append("補金")
        repair_detail_items.append("補金")

    st.write("---")
    eng_choice = st.radio("刻字服務", ["無", "1-5字($50)", "6-12字($200)", "簡易向量圖($100)", "12字以上($400)"], key='eng_choice', horizontal=True)
    if "1-5" in eng_choice:   sub_e += 50;  repair_detail_items.append("刻字1-5字")
    elif "6-12字" in eng_choice: sub_e += 200; repair_detail_items.append("刻字6-12字")
    elif "向量" in eng_choice:  sub_e += 100; repair_detail_items.append("簡易向量圖")
    elif "12字以上" in eng_choice: sub_e += 400; repair_detail_items.append("刻字12字以上")

    eng_custom = st.number_input("自訂刻字金額 ($)", min_value=0, step=50, key='eng_custom')
    sub_e += eng_custom
    if eng_custom > 0: repair_detail_items.append(f"自訂刻字${eng_custom}")

    if repair_detail_items:
        details.append(f"維修[{'、'.join(repair_detail_items)}](${int(sub_e)})")
        simple_details.append(f"維修[{'、'.join(repair_detail_items)}]")
    elif sub_e > 0:
        details.append(f"維修(${int(sub_e)})")
        simple_details.append("維修")
...
with st.expander("🕯️ 3D 出蠟", expanded=False):
    ...
    if scan_fee > 0: details.append(f"3D掃描(${scan_fee})"); simple_details.append("3D掃描")
    if draw_fee > 0: details.append(f"製圖(${draw_fee})"); simple_details.append("製圖")
...
with st.expander("💍 裸石價格", expanded=False):
    ...
    for j in range(bn):
        ...
        cp      = cp_col.number_input("售價/ct",    min_value=0.0, key=f'cp_{j}')
        ...
        if cp > 0 and cw > 0:
            ...
            simple_details.append(f"裸石({ct_type})")
        cost_price = st.number_input("入庫價/ct（記錄用）", min_value=0.0, key=f'cp_cost_{j}', step=100.0)
        if cost_price > 0 and cw > 0:
            bare_cost_note.append(f"({ct_type if cp > 0 else '裸石'}{cw:.3f}ct入庫${int(cost_price)}/ct)")
...
with st.expander("✨ 拋光 / 電鍍", expanded=False):
    ...
    if "鑲口/耳環" in polish_choice:
        sub_d += 50
        if st.checkbox("附加電鍍 +$50", key='p_add_plate'):
            sub_d += 50
    elif "戒指/墜子)、耳環" in polish_choice:
        sub_d += 100
        if st.checkbox("附加電鍍 +$50", key='p_add_plate'):
            sub_d += 50
    elif "完整拋光" in polish_choice:
        sub_d += 200
        if st.checkbox("附加電鍍 +$50", key='p_add_plate'):
            sub_d += 50
    elif "素款項鍊" in polish_choice:
        sub_d += 200
        if st.checkbox("附加電鍍 +$100", key='p_add_plate'):
            sub_d += 100
    elif "手鍊1卡內)、手鐲" in polish_choice:
        sub_d += 300
        if st.checkbox("附加電鍍 +$200", key='p_add_plate'):
            sub_d += 200
    elif "手鍊1卡內 $400" in polish_choice:
        sub_d += 400
        if st.checkbox("附加電鍍 +$200", key='p_add_plate'):
            sub_d += 200
    ...
    # 左側：白金、雙色、噴砂/拉砂
    if ca1.checkbox("白金 +$50",        key='p_white'):   sub_d += 50
    if ca1.checkbox("雙色 +$100",       key='p_double'):  sub_d += 100
    if ca1.checkbox("噴砂、拉砂 +$100", key='p_sand'):    sub_d += 100
    # 右側：其餘
    if ca2.checkbox("大件 +$50",              key='p_wide'):       sub_d += 50
    if ca2.checkbox("過砂紙 +$50",            key='p_sandpaper'):  sub_d += 50
    if ca2.checkbox("執模後粗拋 +$100",       key='p_rough'):      sub_d += 100
    if ca2.checkbox("手鍊1卡內、手鐲粗拋 +$200", key='p_rough_chain'): sub_d += 200

    st.divider()
    custom_plate = st.number_input("單獨吊色 / 自訂金額 ($)", min_value=0, step=10, key='custom_plate')
    sub_d += custom_plate

    if sub_d > 0: details.append(f"拋光電鍍(${int(sub_d)})"); simple_details.append("拋光電鍍")
...
recv_weight = rw_col1.number_input("📥 收貨重量 (錢)", min_value=0.0, step=0.001, format="%.3f", key='recv_weight')
ship_weight = rw_col2.number_input("📤 出貨重量 (錢)", min_value=0.0, step=0.001, format="%.3f", key='ship_weight')

# 數量 + 工時（同一行）
qty_col1, qty_col2, wt_col1, wt_col2 = st.columns([1, 2, 1, 2])
order_qty = qty_col1.number_input("🔢 件數", min_value=1, step=1, value=1, key='order_qty')
if order_qty > 1:
    qty_col2.markdown(f"<div style='padding-top:28px;font-size:14px;'>單件 <b>${int(total_final)}</b> × {order_qty}</div>", unsafe_allow_html=True)

work_min = wt_col1.number_input("⏱ 工時(分)", min_value=0, step=5, key='work_min')
wt_str = ""
if work_min > 0:
    wt_h = work_min // 60
    wt_m = work_min % 60
    if wt_h > 0 and wt_m > 0:   wt_str = f"{wt_h}h {wt_m}m"
    elif wt_h > 0:               wt_str = f"{wt_h} 小時"
    else:                        wt_str = f"{wt_m} 分鐘"
    wt_col2.markdown(f"<div style='padding-top:28px;font-size:14px;'>= <b>{wt_str}</b></div>", unsafe_allow_html=True)

total_final_qty = int(total_final) * order_qty

# ── 拆分工費 / 金料 / 裸石 ──
total_bare   = int(sub_b2) * order_qty
total_gold   = (sub_a_gold + int(resize_gold) + int(repair_gold)) * order_qty
total_labor  = total_final_qty - total_gold - total_bare
...
col_lf, col_gf, col_bf, col_tf = st.columns(4)
col_lf.metric("💼 工費金額", f"${int(total_labor)}")
col_gf.metric("💰 金料金額", f"${int(total_gold)}")
col_bf.metric("💎 裸石金額", f"${int(total_bare)}")
col_tf.metric("🏷️ 總金額",  f"${int(total_final_qty)}")
if order_qty > 1:
    st.markdown(f"<h2 style='text-align: center; color: red;'>當前應收金額: ${int(total_final_qty)}（{order_qty} 件）</h2>", unsafe_allow_html=True)
else:
    st.markdown(f"<h2 style='text-align: center; color: red;'>當前應收金額: ${int(total_final_qty)}</h2>", unsafe_allow_html=True)
...
with col_btn2:
    if st.button("➕ 將此筆加入紀錄", use_container_width=True):
        new_record = {
            ...
            "備註": customer_name or note,
            "件數": order_qty,
            "總金額": int(total_final_qty),
            "單件金額": int(total_final),
            "明細": " | ".join(details) if details else "無項目",
            "精簡明細": "｜".join(simple_details) if simple_details else "",
            "工費金額": int(total_labor),
            "裸石金額": int(total_bare),
            "黃金牌價": g_sell,
            "鉑金牌價": p_sell_pt,
            "工時": wt_str if work_min > 0 else "",
            "收貨重量": f"{recv_weight:.3f}錢" if recv_weight > 0 else "",
            "出貨重量": f"{ship_weight:.3f}錢" if ship_weight > 0 else "",
            "報價模式": "零售維修" if is_retail_repair else ("零售新訂" if is_walkin else "一般"),
            "入庫備注": " ".join(bare_cost_note) if bare_cost_note else "",
            "客戶備註": note,
        }
...
if st.session_state.records:
    ...
    def render_record(idx, rec):
        col_chk, col_card = st.columns([0.5, 9.5])
        is_checked = col_chk.checkbox("", key=f'rec_chk_{idx}',
                                       value=(idx in st.session_state.selected_records))
        if is_checked:
            selected.add(idx)

        date_str   = rec.get("時間", "")
        note_str   = rec.get("備註", "") or "—"
        total_str  = rec.get("總金額", 0)
        unit_str   = rec.get("單件金額", "")
        qty_str    = rec.get("件數", 1)
        labor_str  = rec.get("工費金額", "")
        bare_str   = rec.get("裸石金額", "")
        detail_raw  = rec.get("明細", "")
        detail_lines = []
        for part in detail_raw.split(" | "):
            part = part.strip()
            if not part or part == "無項目":
                continue
            # 把項目內的 | 也換行顯示（執模的工費|金料 分開）
            part = part.replace("|", "\n　　")
            detail_lines.append(part)
        detail_str = "\n　".join(detail_lines)
        mode_str   = rec.get("報價模式", "")
        time_str   = rec.get("工時", "")
        gold_str   = rec.get("黃金牌價", "")
        plat_str   = rec.get("鉑金牌價", "")
        cost_str   = rec.get("入庫備注", "")
        note2_str  = rec.get("客戶備註", "")

        qty_label = f"　× {qty_str} 件　單件 ${unit_str}" if qty_str and int(qty_str) > 1 else ""
        meta_parts = []
        if labor_str != "": meta_parts.append(f"工費 ${labor_str}")
        if bare_str not in ("", 0, "0"): meta_parts.append(f"裸石 ${bare_str}")
        if mode_str and mode_str != "一般": meta_parts.append(f"【{mode_str}】")
        if time_str: meta_parts.append(f"⏱ {time_str}")
        recv_w = rec.get("收貨重量", "")
        ship_w = rec.get("出貨重量", "")
        if recv_w or ship_w:
            weight_str = f"📥{recv_w}" if recv_w else ""
            weight_str += f"　📤{ship_w}" if ship_w else ""
            meta_parts.append(weight_str)
        if cost_str: meta_parts.append(f"入庫：{cost_str}")
        if note2_str and note2_str != note_str: meta_parts.append(f"備註：{note2_str}")
        meta_line = "　".join(meta_parts)
        gold_line = f"金價：黃金 ${gold_str}　鉑金 ${plat_str}" if (gold_str or plat_str) else ""

        card_html = f"""
<div style="background:#1e1e2e;border:1px solid #3a3a5c;border-radius:10px;padding:12px 16px;margin-bottom:8px;line-height:1.7;">
  <div style="font-size:18px;font-weight:700;color:#e0e0ff;margin-bottom:4px;">
    #{idx+1}　{date_str}　<span style="color:#ffd700;">${total_str}</span>{qty_label}
    {"　<span style='font-size:13px;color:#aaa;'>" + note_str + "</span>" if note_str != "—" else ""}
  </div>
  {"<div style='font-size:13px;color:#b0b0d0;margin-bottom:2px;'>" + meta_line + "</div>" if meta_line else ""}
  <div style="font-size:13px;color:#c8c8e8;white-space:pre-wrap;margin-top:4px;">　{detail_str}</div>
  {"<div style='font-size:11px;color:#666;margin-top:6px;'>" + gold_line + "</div>" if gold_line else ""}
</div>"""
        col_card.markdown(card_html, unsafe_allow_html=True)

    # 最新3筆直接顯示，其餘折疊
    total_count = len(records)
    show_direct = min(3, total_count)
    for idx in range(total_count - show_direct, total_count):
        render_record(idx, records[idx])

    if total_count > 3:
        with st.expander(f"📂 顯示更早的 {total_count - 3} 筆紀錄"):
            for idx in range(0, total_count - 3):
                render_record(idx, records[idx])
    ...
    with btn1:
        if st.button("🗑️ 刪除勾選項目", use_container_width=True):
            if selected:
                new_records = [r for i, r in enumerate(records) if i not in selected]
                st.session_state.records = new_records
                st.session_state.selected_records = set()
                save_records_to_github(new_records, st.session_state.gh_sha)
                st.rerun()
    with btn2:
        if st.button("☑️ 全選 / 取消全選", use_container_width=True):
            if len(selected) == len(records):
                ...
                st.session_state.selected_records = set(range(len(records)))
            st.rerun()
    with btn3:
        if st.button("🗑️ 清空所有紀錄", use_container_width=True):
            save_records_to_github([], st.session_state.gh_sha)
    ...
    export_indices = sorted(selected) if selected else list(range(len(records)))
    export_records = [records[i] for i in export_indices]

    # ── 完整 Excel 匯出 ──
    df_full = pd.DataFrame(export_records)
    out_full = io.BytesIO()
    with pd.ExcelWriter(out_full, engine='openpyxl') as writer:
        df_full.to_excel(writer, index=True, index_label="編號", sheet_name='完整報價紀錄')
    out_full.seek(0)
    excel_bytes = out_full.read()

    st.download_button(
        label=f"📥 完整匯出（{'勾選' if selected else '全部'}）",
        data=excel_bytes,
        file_name=f"完整報價_{datetime.now().strftime('%m%d')}.xlsx",
